...
then
	if ./configure --no-regex --no-pcre2 --openssl --h2 \
	               --fuzz="$LIB_FUZZING_ENGINE"
	then
		NXT_FUZZ_H2=yes
	else
		echo "oss-fuzz.sh: configure --openssl --h2 failed (see above);" \
		     "building without fuzz_http_h2p" >&2
	fi
else
	echo "oss-fuzz.sh: libnghttp2 not found; building without fuzz_http_h2p" >&2
fi
