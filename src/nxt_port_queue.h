...
nxt_port_queue_recv(nxt_port_queue_t volatile *q, void *p)
{
     ...
     * is sized for NXT_PORT_QUEUE_MSG_SIZE.  Read through a volatile cast
     * so the compiler cannot fold the bound-check and the memcpy length
     * into two separate reads of qi->size that a concurrent peer write
     * could make disagree.
     */
