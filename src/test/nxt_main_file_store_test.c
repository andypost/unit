...
nxt_main_file_store_test(nxt_thread_t *thr)
{
    ...
    /*
     * A certificate bundle is replaced under its own name, with the
     * temporary beside it under a reserved dot name.  A shorter bundle
     * after a longer one must not keep the tail of the longer one, which
     * the old in-place open without O_TRUNC left behind.
     */
