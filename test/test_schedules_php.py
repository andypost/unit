...
This is the PHP-specific companion to test_schedules.py, which covers
validation and the run mechanics (overlap, timeout) against a Python
application. Here the point is PHP itself: the request a PHP script sees,
and fastcgi_finish_request() the way Drupal's "automated_cron" uses it,
through test/php/schedule/index.php. See
...
def put_run(schedules):
    assert 'success' in client.conf(run_conf(schedules))
